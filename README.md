```markdown
# Платформа мониторинга погоды и качества воздуха

**Платформа мониторинга окружающей среды с обнаружением аномалий, построенная на современном стеке data engineering.**
...
└─────────────┘     └──────────────┘     └─────────────────┘
                    ...
                    ┌──────┴──────┐        ┌──────┴──────┐
                    │   Airflow    │        │   Grafana    │
                    │ (Оркестратор) │        │ (Дашборды)   │
                    └─────────────┘        └─────────────┘
```

**Источники данных:**
- [Open-Meteo API](https://open-meteo.com/) — погодные данные (температура, давление, влажность, скорость ветра)
...
|---|---|---|
| `check_locations` | Каждые 16 мин | Обходит все отслеживаемые локации, запускает полный пайплайн |
| `sync_to_clickhouse` | Каждые 5 мин | Инкрементально синхронизирует погоду и качество воздуха в ClickHouse |
...
- Данные о погоде быстро накапливаются: 10 локаций × 24 проверки/день × 365 дней = 87 600 строк/год
- С 5 000 локаций: 43,8 млн строк/год
...
MIT — свободно используйте, модифицируйте и учитесь на этом проекте.

---

*Создано как самообразовательный проект для входа в data engineering. Если вы нанимаете стажёров — я готов учиться дальше.*
```
