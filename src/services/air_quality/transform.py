...
def transform_air_quality(raw_data: Dict) -> Optional[Dict]:
    """
...
def collect_aq(db: Session, location: LocationToTrack) -> Optional[dict]:
    lat = location.lat
    lon = location.lon
    aq = extract_air_quality(lon=lon, lat=lat)
    if aq:
        raw_aq = collect_raw_aq(db, aq)
        structured_aq = save_structured_aq(db, raw_aq, location)
